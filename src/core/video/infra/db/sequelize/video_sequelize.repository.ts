...
export class VideoSequelizeRepository implements IVideoRepository {
	...
	orderBy = {
		mysql: {
			name: (sort_dir: SortDirection) => `binary ${this.videoModel.name}.\`name\` ${sort_dir}`
		}
	};
	relations_include = [
		'categories_id',
	...
	async bulkInsert(entities: Video[]): Promise<void> {
		const models = entities.map(VideoModelMapper.toModelProps);
	...
	async findAll(): Promise<Video[]> {
		...
		return models.map(VideoModelMapper.toEntity)
	}
	...
	async findByIds(ids: VideoId[]): Promise<Video[]> {
		...
		return models.length ? models.map(VideoModelMapper.toEntity) : []
	}
	...
	async update(entity: Video): Promise<void> {
		...
		// Primeiro fazemos a deleção de tudo
		await Promise.all([
			...model.image_medias.map(
				i => i.destroy({ transaction: this.uow.getTransaction() })
			),
			...model.audio_video_medias.map(
				i => i.destroy({ transaction: this.uow.getTransaction() })
			),
			model.$remove(
				'categories',
			...
			model.$remove(
				'genres',
				model.genres_id.map(g => g.genre_id),
				{ transaction: this.uow.getTransaction() }
			),
			model.$remove(
				'cast_members',
		...
		// E então o insert de td mundo de novo
		await Promise.all([
			...image_medias.map(
				i => model.$create('image_media', i.toJSON(), {
					transaction: this.uow.getTransaction()
				})
			),
			...audio_video_medias.map(
				i => model.$create('audio_video_media', i.toJSON(), {
					transaction: this.uow.getTransaction()
				}),
			),
			model.$add(
	...
	async search(props: VideoSearchParams): Promise<VideoSearchResult> {
		...
		if (
			...
			if (props.filter.categories_id) {
				wheres.push({
					...
					rawCondition: `${videoCategoryTableName}.title IN (:categories_id)`,
				});
			}
			...
			if (props.filter.genres_id) {
				wheres.push({
					field: 'genres_id',
					value: props.filter.genres_id.map(g => g.id),
					get condition() {
						return {
							['$genres_id.genre_id$']: {
								[Op.like]: this.value
					...
					rawCondition: `${videoGenreTableName}.title IN (:genres_id)`,
				});
			}
			...
			if (props.filter.cast_members_id) {
				wheres.push({
					...
					rawCondition: `${videoCastMemberTableName}.title IN (:cast_members_id)`,
				});
			}
		}
		...
		const query = [
			...
			props.filter?.genres_id
				...
				: '',
			props.filter?.cast_members_id
				? `INNER JOIN ${videoCastMemberTableName} ON ${videoAlias}.\`video_id\` = ${videoCastMemberTableName}.\`cast_member_id\``
				: '',
			wheres.length
		...
		const [idResult] = await this.videoModel.sequelize!.query(
			query.join(' '),
			{
		...
		return new VideoSearchResult({
			items: models.map(VideoModelMapper.toEntity),
	...
	async existsById(ids: EntityId[]): Promise<{ exists: EntityId[]; not_exists: EntityId[] }> {
		...
		const existsVideoIds = existsVideoModels.map(m => new VideoId(m.video_id));
